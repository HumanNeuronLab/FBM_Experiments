...
def onetrial(win, stim, fix, timing, file_name, trial_number, block_number, is_image=False, is_text=False, is_sound=False, disp_size=0,serial_porting = 0):
    ...
    if is_sound: stim_sound = sound.Sound(stim)
    if not is_text:
    ...
    base_time,cue_time = timing[0][0],timing[1][0]
    fix.draw()
    win.flip()
...
def run_experiment(initial_data):
    ...
    green_fix = visual.TextStim(win=win, text="+", pos=[0, 0], color='green', height=35)

    experiments = [
        ('Picture Naming', 'picture_naming', '*.png', True, False, False, [1.1, 1.2, 1.3, 1.4],),
        ('Auditory Definition', 'auditory_naming', '*.wav', False, True, False, [1.1, 1.2, 1.3, 1.4]),
        ('Sentence Completion', 'reading_completion', '*.txt', False, False, True, [1.1, 1.2, 1.3, 1.4]),
        ('Motor Tasks', 'motor_tasks', green_fix, False, True, False,[2.5,3,3.5],[3]),
        ('Category Localizer (Auditory)', 'auditory_localizer', '*.wav', False, False, True,[0.8, 0.9, 1, 1.1],[2]),
        ('Category Localizer (Visual)', 'visual_localizer', '*.png', True, False, False,[0.8, 0.9, 1, 1.1],[0.8]),
    ]
    ...
    for exp_name, folder, file_pattern, is_image, is_text, is_sound in experiments:
        if info_experiment[exp_name]['Run']:
            ...
            for i, stim in enumerate(stim_files[:3]):
                onetrial(win, stim, fix, TIMING, file_name, i+1, 0, is_image=is_image, is_text=is_text, is_sound=is_sound, disp_size=info_displayscreen["Resolution"])

            for i, stim in enumerate(stim_files[3:info_experiment[exp_name]['Number of Trials']]):
                onetrial(win, stim, fix, TIMING, file_name, i+1, 1, is_image=is_image, is_text=is_text, is_sound=is_sound, disp_size=info_displayscreen["Resolution"])
    win.close()
