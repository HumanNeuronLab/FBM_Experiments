...
def get_base_dir() -> Path:
    ...
    - normal run: folder containing this .py
    - PyInstaller onefile: the temporary _MEIPASS folder
    """
...
def run_motor_mapping(serial_port, experiment_settings):
    ...
    file_name = str(RESULTS_DIR / f"sub-{subject_id}_task-motormapping_datetime-{timestamp}.tsv")
    ...
    for block_name, baseline_durations, stim_durations in blocks:
        ...
        instructions_image = visual.ImageStim(win, image=str(IMAGES_DIR / f"{block_name[0:4]}.png"),size=(300, 300),pos=(0, -250))
        instructions.draw()
