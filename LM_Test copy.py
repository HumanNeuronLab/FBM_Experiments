...
# port = serial.Serial('COM3',9600, timeout=5) #COM4 is the right one
# port.readData
...
def onetrial(mywin,Stim,fix,Timing,FileName,TrialNumber,BlockNumber,isImage=False,isText=False,isSound = False):
    ...
    if isImage: # if image
        ...
        StimVisual.draw()
        mywin.flip()
        # port.setData(port_value)
    elif isText:
        StimVisual=visual.TextStim(win=mywin,text="",color=[0,0,0],height=50)
    ...
    ## 3: RESPONSE
    tic=time.time()
    ResponseText=visual.TextStim(win=mywin,text="",color=[0,0,0],height=100)
...
while True:
    ...
    if InitialData==['', '', 'M', 'R', 0.5]:# Cancel if press
        Exp=False
        break
    else:
        ...
        if os.path.isfile(FileName):
            ...
            break
if Exp:
        mywin=visual.Window([1000,800], pos=[0,0], monitor="default",waitBlanking=True,units="pix",color='white',fullscr=True,allowGUI=True)
        fix=visual.TextStim(win=mywin,text="+",pos=[0,0], color=[0,0,0],height=30)
        ...
        # Exit=False
        # IntroText=visual.TextStim(win=mywin,text="",color=[0,0,0])
        # IntroText.setText(text='Auditory Naming:\n Respond with the word that best explains the sentence when the question mark appears.\n\nPress x for bad trial\nPress space to continue\nPress q to quit')
        # IntroText.draw()
        # mywin.flip()
        ...
        ResponseText=visual.TextStim(win=mywin,text="",color=[0,0,0],height=50)
